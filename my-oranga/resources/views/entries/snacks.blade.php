...
<div class="container">
    ...
    <div class="row d-flex justify-content-right text-right">
        <div class="col-md-12 pb-4">
            <a href="{{route('snacks.create')}}" class="btn btn-outline-success">Add Snack <i class="fa fa-plus fa-1x pl-2"></i></a>
        </div>
    </div>
<div id="activities" class="table-responsive">
    <table class="table">
        <thead>
            ...
            <tbody>
            @foreach($snacks as $entry)
                <tr>
                    <td>{{$entry->date}}</td>
                    <td>{{$entry->item}}</td>
                    <td>{{$entry->calories}}</td>
                    <td>{{$entry->kj}}</td>
                    <td><a href="{{route('snacks.edit', $entry->id)}}" class="btn btn-primary tbl-edit-btn"><i class="fas fa-edit fa-1x"></i></a></td>
                </tr>
            @endforeach
        </tbody>
    </table>
